...
panel.preparePage = function() {
    if (!panel.token) return window.location = '/';
    panel.verifyToken(panel.token, !0)
}
...
panel.prepareDashboard = function() {
    ...
    document.getElementById('adminManage').addEventListener('click', function() {
        panel.setActiveMenu(this)
    });
    ...
    axios.get(url).then(function(response) {
        ...
        if (panel.filesView === 'thumbs') {
            ...
            for (var item of response.data.files) {
                ...
                else div.innerHTML = `<a href="${item.file}" target="_blank"><h1 class="title">.${item.file.split('.').pop()}</h1></a>`;
...
//if (panel.username === 'root') {
panel.admin_management = function() {
...
panel.changeToken = function() {
    axios.get('/api/tokens').then(function(response) {
        ...
        container.className = "container";
        container.innerHTML = `
			<h2 class="subtitle">API Key Management</h2>
			<label class="label">Your current key:</label>
			<p class="control has-addons">
				<input id="token" readonly class="input is-expanded" type="text" placeholder="Your token" value="${response.data.token}">
				<a id="getNewToken" class="button is-primary">Change key</a>
...
panel.changePassword = function() {
    ...
    container.innerHTML = `
		...
		<p class="control has-addons">
			...
			<a id="sendChangePassword" class="button is-primary">Set password</a>
		</p>
	`;
    panel.page.appendChild(container);
